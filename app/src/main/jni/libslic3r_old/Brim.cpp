...
static float max_brim_width(const SpanOfConstPtrs<PrintObject> &objects)
{
    ...
    return float(std::accumulate(objects.begin(), objects.end(), 0.,
                                 [](double partial_result, const PrintObject *object) {
                                     return std::max(partial_result, object->config().brim_type == btNoBrim ? 0. : (object->config().brim_type == btAutoBrim ? std::max((float)object->config().brim_width.value, 5.0f) : object->config().brim_width.value));
                                 }));
}
...
// Auto Brim logic ported for SliceBeam
struct ExPolyProp
{
    double aera = 0;
    Vec2d  centroid;
...
static float configBrimWidthAuto(const PrintObject *object, const ExPolygons &expolys)
{
    ...
    double bboxX = bbox2.size()(0);
    double bboxY = bbox2.size()(1);
    double thermalLength = sqrt(bboxX * bboxX + bboxY * bboxY) * SCALING_FACTOR;
    double thermalLengthRef = thermalLength; // Approx

    double adhesion = 1.0;
    for (const std::string& type : object->print()->config().filament_type.values) {
        if (type == "ABS" || type == "ASA") adhesion = 2.0;
    }

    double maxSpeed = 100.0; // Approx max speed
    ...
    // Limit brim width
    if (brim_width > 0 && brim_width < 5.0) brim_width = 5.0;

    return (float)brim_width;
}

static float get_auto_brim_width(const PrintObject *object, const ExPolygons &bottom_layer_expolys, float default_brim_width)
{
    if (object->config().brim_type != btAutoBrim) return default_brim_width;
    float auto_width = configBrimWidthAuto(object, bottom_layer_expolys);
    return std::max(auto_width, default_brim_width);
}
                                            ...
                                            const float                    no_brim_offset)
{
    ...
    for(size_t print_object_idx = 0; print_object_idx < print.objects().size(); ++print_object_idx) {
        const PrintObject *object            = print.objects()[print_object_idx];
        BrimType     brim_type         = object->config().brim_type.value;
        if (brim_type == btAutoBrim) brim_type = brim_width > 0 ? btOuterOnly : btNoBrim;
        const float        brim_separation   = scale_(object->config().brim_separation.value);
        const float        brim_width        = scale_(get_auto_brim_width(object, bottom_layers_expolygons[print_object_idx], object->config().brim_width.value));
...
ExtrusionEntityCollection make_brim(const Print &print, PrintTryCancel try_cancel, Polygons &islands_area)
{
    ...
    const bool could_brim_intersects_skirt = std::any_of(print.objects().begin(), print.objects().end(), [&print](const PrintObject *object) {
        ...
        return (bt == btOuterOnly || bt == btOuterAndInner || bt == btAutoBrim) && print.config().skirt_distance.value < object->config().brim_width;
    });
