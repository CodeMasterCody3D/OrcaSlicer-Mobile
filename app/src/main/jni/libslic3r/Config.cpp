...
ConfigSubstitutions ConfigBase::load(const std::string &file, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    if (is_gcode_file(file))
        ...
        // SliceBeam stores its merged slicing config as a flat PrusaSlicer-style .ini; OrcaSlicer
        // disabled the .ini loader (it uses JSON), so re-enable it for these files.
