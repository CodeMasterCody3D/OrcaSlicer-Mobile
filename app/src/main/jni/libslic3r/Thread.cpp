...
std::optional<std::string> get_current_thread_name()
{
	...
	// pthread_getname_np() is only available on Android API 26+. For the current thread
	// prctl(PR_GET_NAME) is equivalent and works on all supported API levels.
	return std::string(prctl(PR_GET_NAME, buf) == 0 ? buf : "");
#else
